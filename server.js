...
// PocketBase client for server-side operations
  ...
  if (req.path === '/sepa-dd' || req.path === '/sepa-dd/') {
    // Attempt to read the built index.html from dist; fall back to the
    // repository root index.html if dist is not present (useful in dev).
    const distIndex = path.join(__dirname, 'dist', 'index.html');
    const fallbackIndex = path.join(__dirname, 'index.html');
    const serveCustomized = async () => {
      try {
        let html = '';
        try {
        ...
        // Replace or insert meta tags for the /sepa-dd route. We keep it
        // simple: replace <title> and some common meta tags if present, else
        // insert them in the <head>.
        ...
        const sepaCanonical = 'https://forms.nexusventures.eu/sepa-dd';
        ...
        const upsertMetaTag = (tag) => {
          const nameOrProp = tag.match(/<(meta[^>]+)>/i)?.[1] ?? tag;
          const keyMatch = nameOrProp.match(/name=\"([^\"]+)\"/) || nameOrProp.match(/property=\"([^\"]+)\"/);
        ...
        upsertMetaTag(`<meta name="description" content="${sepaDescription}">`);
        upsertMetaTag(`<meta property="og:description" content="${sepaDescription}">`);
        upsertMetaTag(`<meta property="og:title" content="${sepaTitle}">`);
        upsertMetaTag(`<meta name="twitter:title" content="${sepaTitle}">`);
        upsertMetaTag(`<meta name="twitter:description" content="${sepaDescription}">`);
        upsertMetaTag(`<meta property="og:image" content="${ogImage}">`);
        upsertMetaTag(`<meta name="twitter:image" content="${ogImage}">`);

        // canonical link
        if (/\<link[^>]+rel=\"canonical\"[^>]*>/i.test(html)) {
          html = html.replace(/\<link[^>]+rel=\"canonical\"[^>]*>/i, `<link rel="canonical" href="${sepaCanonical}" />`);
        } else {
          html = html.replace(/<head([^>]*)>/i, `<head$1>\n    <link rel="canonical" href="${sepaCanonical}" />`);
        }
        ...
        console.error('Error while preparing SEPA-specific HTML', e);
        // fallback to normal index file send
        return res.sendFile(path.join(__dirname, 'dist', 'index.html'));
      }
    };

    return serveCustomized();
  }
