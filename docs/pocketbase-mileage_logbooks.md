# PocketBase: mileage_logbooks collection

This document contains an importable PocketBase collection schema and instructions for creating the `mileage_logbooks` collection used by the Nexus Ventures mileage logbook forms.

## What this collection does
- Stores submitted mileage logbooks with the generated PDF copy.
...
- `docs/pb_mileage_logbooks_import.json` — importable JSON for the PocketBase Collections import tool. Replace `_pb_users_auth_` if necessary.

## How to import
1. Open your PocketBase Admin UI (usually at `http://127.0.0.1:8090/_/`).
2. Go to Collections -> Import.
3. Paste the contents of `docs/pb_mileage_logbooks_import.json` or upload the file.
...
| name    | text        | Yes      | Driver/submitter name             |
| email   | email       | Yes      | Driver/submitter email            |
...
---
Generated: PocketBase import JSON is in `docs/pb_mileage_logbooks_import.json`.
